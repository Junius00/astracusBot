...
async def buy_powerup_card(update, context):
    ...
    r_options = [", ".join([f'{p} {r}' for p, r in zip(
        pup.get_price_list(), r_set)]) for r_set in r_sets]
...
async def use_powerup_card(update, context):
    ...
    pups_str = 'Powerup Cards Available:\n' + \
        '\n'.join([f'({i+1}) {pup.name}: {pup.desc}' for i,
                  pup in enumerate(pups)])
...
async def place_collateral_buildings(update, context):
    ...
    async def place_building(b):
        choices = []
        if b.name == B_VILLAGE:
            choices += g_env.MAP.get_possible_choices(
                og, b, override_building_type=B_HOUSE)
